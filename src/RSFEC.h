...
namespace RS {
  ...
  namespace gf {
    ...
    inline uint8_t mul(uint16_t x, uint16_t y) {
      ...
      return pgm_read_byte_near(exp + pgm_read_byte_near(log + x) + pgm_read_byte_near(log + y));
    }
    ...
    inline uint8_t div(uint8_t x, uint8_t y) {
      ...
      return pgm_read_byte_near(exp + (pgm_read_byte_near(log + x) + 255 - pgm_read_byte_near(log + y)) % 255);
    }
    ...
    inline uint8_t pow(uint8_t x, intmax_t power) {
      // intmax_t i = log[x];
      intmax_t i = pgm_read_byte_near(log + x);
      i *= power;
      i %= 255;
      if (i < 0) i = i + 255;
      // return exp[i];
      return pgm_read_byte_near(exp + i);
    }
    ...
    inline uint8_t inverse(uint8_t x) {
      ...
      return pgm_read_byte_near(exp + 255 - pgm_read_byte_near(log + x));
    }
